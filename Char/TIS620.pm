...
######################################################################
#
# Char::TIS620 - Source code filter to escape TIS-620 script
#
# http://search.cpan.org/dist/Char-Char::TIS620/
#
...
BEGIN { CORE::require Char::Etis620; }
...
my $function_ord;         # ord()   to ord()   or Char::TIS620::ord()
my $function_ord_;        # ord     to ord     or Char::TIS620::ord_
my $function_reverse;     # reverse to reverse or Char::TIS620::reverse
my $function_getc;        # getc    to getc    or Char::TIS620::getc
...
END
    ...
    print Char::TIS620::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Etis620::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::TIS620::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Etis620::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::TIS620::escape_script {
    ...
    # read TIS-620 script
    my $fh = gensym();
    Char::Etis620::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Etis620(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Etis620 %s.0;\n", $Char::TIS620::VERSION); # require run-time routines version

        # use Char::TIS620 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::TIS620 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::TIS620::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/TIS620.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/TIS620.pm $version, but @{[__FILE__]} is version $Char::TIS620::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Etis620.pm $version, but \\\$Char::Etis620::VERSION is \$Char::Etis620::VERSION" if \$Char::Etis620::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::TIS620::VERSION) {
                    die "Script $0 required Char/TIS620.pm $version, but @{[__FILE__]} is only version $Char::TIS620::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::TIS620::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::TIS620::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::TIS620::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::TIS620::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::TIS620::re_t=%s; while($Char::TIS620::re_t =~ %s){%s local $^W=0; local $Char::TIS620::re_r=%s; %s$Char::TIS620::re_t="$`$Char::TIS620::re_r$'"; pos($Char::TIS620::re_t)=length "$`$Char::TIS620::re_r"; } return $Char::TIS620::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::TIS620::re_r=eval $Char::TIS620::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::TIS620::re_r=%s; %s"%s$Char::TIS620::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::TIS620::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::TIS620::re_r=%s; %s%s="$`$Char::TIS620::re_r$'"; pos(%s)=length "$`$Char::TIS620::re_r"; $Char::TIS620::re_n++} return %s$Char::TIS620::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::TIS620::re_r=%s; %s%s="%s$Char::TIS620::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::TIS620::re_r=%s; %s%s="%s$Char::TIS620::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::TIS620::re_r=eval $Char::TIS620::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/TIS620.pm and Char/Etis620.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::TIS620;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::TIS620;
  use Char::TIS620 ver.sion;             --- require minimum version
  use Char::TIS620 ver.sion.0;           --- expects version (match or die)

  # "no Char::TIS620;" not supported

  or

  $ perl Char/TIS620.pm TIS-620_script.pl > Escaped_script.pl.e
...
Char::TIS620 software is "middleware" between perl interpreter and your Perl script
written in TIS-620.
...
By "use Char::TIS620;", it automatically interpret your script as TIS-620. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/TIS620.pm, Char/Etis620.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Etis620.pm
works well.
...
=head1 Software Composition
   ...
   Char/TIS620.pm               --- source code filter to escape TIS-620
   Char/Etis620.pm              --- run-time routines for Char/TIS620.pm
...
You need write 'use Char::TIS620;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::TIS620;
  ---------------------------------

=head1 Calling 'Char::Etis620::ignorecase()' (Char/TIS620.pm provides)

Char/TIS620.pm applies calling 'Char::Etis620::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Etis620::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Etis620.pm provides)
...
Definitions in Char/Etis620.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/TIS620.pm provides)

Char/TIS620.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/TIS620.pm and Char/Etis620.pm provide)

Insert 'Char::Etis620::' at head of function name. Char/Etis620.pm provides your script Char::Etis620::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Etis620::lc        Character
  lcfirst     Char::Etis620::lcfirst   Character
  uc          Char::Etis620::uc        Character
  ucfirst     Char::Etis620::ucfirst   Character
  fc          Char::Etis620::fc        Character
  chr         Char::Etis620::chr       Character
  glob        Char::Etis620::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Subroutines (Char/TIS620.pm provides)

Char/TIS620.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Char::TIS620 and change
'use utf8;' to 'use Char::TIS620;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Etis620.pm provides these
  functions.
...
Char::TIS620 software remains one language and one interpreter.
