...
my $qq_substr  = qr{(?: TIS620::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or TIS620::ord()
my $function_ord_;        # ord     to ord     or TIS620::ord_
my $function_reverse;     # reverse to reverse or TIS620::reverse
my $function_getc;        # getc    to getc    or TIS620::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'TIS620::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'TIS620::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'TIS620::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'TIS620::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $TIS620::re_t=%s; while($TIS620::re_t =~ %s){%s local $^W=0; local $TIS620::re_r=%s; %s$TIS620::re_t="$`$TIS620::re_r$'"; pos($TIS620::re_t)=length "$`$TIS620::re_r"; } return $TIS620::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$TIS620::re_r=eval $TIS620::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $TIS620::re_r=%s; %s"%s$TIS620::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $TIS620::re_n=0; while(%s =~ %s){%s local $^W=0; local $TIS620::re_r=%s; %s%s="$`$TIS620::re_r$'"; pos(%s)=length "$`$TIS620::re_r"; $TIS620::re_n++} return %s$TIS620::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $TIS620::re_r=%s; %s%s="%s$TIS620::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $TIS620::re_r=%s; %s%s="%s$TIS620::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$TIS620::re_r=eval $TIS620::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Etis620.pm
works well.
...
=head1 Calling 'Etis620::ignorecase()' (TIS620.pm provides)

TIS620.pm applies calling 'Etis620::ignorecase()' instead of /i modifier.
...
Definitions in Etis620.pm.
  ...
  ${Etis620::not_lower_i}    qr{(?:[\x00-\xFF])}
  ...
  ${Etis620::not_upper_i}    qr{(?:[\x00-\xFF])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (TIS620.pm provides)

TIS620.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (TIS620.pm and Etis620.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (TIS620.pm provides)

TIS620.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/TIS620 and change
'use utf8;' to 'use TIS620;' in its. You need help yourself for now.
